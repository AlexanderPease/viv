        ...
        if session_id:
            ...
            if not entry:
                try:
                    ...
                    logging.warning('Monthly Quote: %s' % monthly_quote)
        return
    ...
    def post(self):
        ...
        if entry:
            try:
                ...
                entry.email = email
                entry.save()
                logging.info("Saved name and email to entry of session_id: %s" % entry.session_id)
            except:
                logging.info("Could not add name or email to entry of session_id: %s" % entry.session_id)
        return
